...
void GameServer::sendTurn()
{
    cout << "Player turn is " << playerTurn << endl;
    for (int i = 0; i < 6; i++)
    {
        cout << players[i].character << endl;
    }
...
void GameServer::updatePlayers()
{
    ...
    sf::Packet sendPos;
    sendPos << receivedPlayer.character << receivedPlayer.column << receivedPlayer.row << !receivedPlayer.isTurn;
    sendAll(sendPos);
